...
bottom_develop (CRConst (RConst _ ps) _) (q, r) = project ps [q]
    where project [] _
              = []
          project ((p', r'):ps') qs
...
bottom_steps :: (Signature s, Variables v, RewriteSystem s v r)
    => CReduction s v r -> Step s v -> [Step s v]
bottom_steps s p = concat (bottom_develop s p)
...
bottom_modulus s@(CRConst _ phi) p@(_, rule) n
    = length (concat (take (phi (n + left_height rule)) (bottom_develop s p)))
...
right_develop :: (Signature s, Variables v, RewriteSystem s v r)
    => CReduction s v r -> Step s v -> [[Step s v]]
right_develop (CRConst (RConst _ ps) phi) (q, r) = project ps [q] 0 0
    where project _ [] _ _
              = []
          project ps' qs n d
...
right_steps :: (Signature s, Variables v, RewriteSystem s v r)
    => CReduction s v r -> Step s v -> [Step s v]
right_steps t p = concat (right_develop t p)
...
right_modulus :: (Signature s, Variables v, RewriteSystem s v r)
    => CReduction s v r -> Step s v -> Modulus
right_modulus s p n = length (concat (take (n + 1) (right_develop s p)))
